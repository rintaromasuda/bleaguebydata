...
showAttendanceTrend <- function(team) {
  longName <-subset(b.teams, NameShort == team & Season == b.current.season)$NameLong
  
  df.team <- subset(df, TeamName == team)
  
  ggplot() +
    geom_line(data = subset(df.team, Season == "2016-17"),
              aes(x = (Date + 365 + 365 + 365),
                  y = Attendance,
                  color = "2016-17"),
              size = 1,
              linetype = 1) +
    ...
    geom_label_repel(data = subset(df.team, Season == "2016-17" & (DayOfWeek != "日" & DayOfWeek != "土")),
                     aes(x = Date + 365 + 365 + 365, y = Attendance, label = DayOfWeek),
    ...
    geom_label_repel(data = subset(df.team, Season == "2018-19" & (DayOfWeek != "日" & DayOfWeek != "土")),
                     ...
                     color ="blue",
                     segment.size  = 0.05) +
    geom_line(data = subset(df.team, Season == "2019-20"),
              aes(x = (Date),
                  y = Attendance,
                  color = "2019-20"),
              alpha = 0.8,
              size = 3,
              linetype = 1) +
    # geom_point(data = subset(df.team, Season == "2019-20"),
    #            aes(x = (Date),
    #                y = Attendance),
    #            color = "blue") +
    # geom_label_repel(data = subset(df.team, Season == "2019-20" & (DayOfWeek != "日" & DayOfWeek != "土")),
    #                  aes(x = Date, y = Attendance, label = DayOfWeek),
    #                  nudge_y = 100 - subset(df.team, Season == "2019-20" & (DayOfWeek != "日" & DayOfWeek != "土"))$Attendance,
    #                  color ="blue",
    #                  segment.size  = 0.05) +
    ylab("観客動員数（人）") +
    xlab("開催日") +
    ggtitle(paste0(longName, "のホームゲーム観客動員数")) +
    scale_x_date(date_labels = "%m/%d", limits = c(as.Date("2019-09-28"), as.Date("2020-05-07"))) +
