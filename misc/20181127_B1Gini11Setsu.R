...
df_201819 <- read.csv("B1_201819_11setsu_BoxScore.csv",
                      sep = ",",
                      header = TRUE)
colnames(df_201819)
dim(df_201819)
...
ggplot() +
  geom_boxplot(data = subset(df_Gini, TEAM != "島根" & TEAM != "西宮"), aes(x = TEAM, y = GINI, color = SEASON)) +
  xlab("") +
  ylab("プレイタイムのジニ係数") +
  ggtitle("2017-18レギュラーシーズン60試合と2018-19レギュラーシーズン第11節までの比較") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  guides(color=guide_legend(title="シーズン"))

ggsave("gini.jpeg", width = 8, height = 5)

ggplot() +
  geom_boxplot(data = subset(df_Gini, TEAM != "島根" & TEAM != "西宮" & TEAM != "秋田" & TEAM != "福岡"), aes(x = SEASON, y = GINI, color = SEASON)) +
  xlab("") +
  ylab("プレイタイムのジニ係数") +
  ggtitle("2017-18レギュラーシーズン60試合と2018-19レギュラーシーズン第11節までの比較") +
  guides(colour=FALSE) +
...
ggsave("gini_total.jpeg", width = 8, height = 5)
