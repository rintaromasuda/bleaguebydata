...
View(df_cleaned[df_cleaned$PID == 8493,])
View(df_cleaned[df_cleaned$PID == 8701 & df_cleaned$YEAR == 2017,])
View(df_cleaned[df_cleaned$PID == 15811 & df_cleaned$YEAR == 2017,])
...
cumplot(c(8657,8449,8487,8592,8475),
        "3FGM",
        "1試合平均スリーポイント成功数の推移（2017-18シーズン）",
        "1試合平均スリーポイント成功数")
...
cumplot2 <- function(ids, stat, stat2, title, ylab) {
  ...
  ggplot(df, aes(SEQ, CUM/CUM2, col = PLAYER)) +
    geom_line(size = .8) +
    theme_bw() +
    ylab(ylab) +
    xlab("試合数") +
